@extends('HRM/default') {{-- Page title --}} @section('title') Kuota KMP Form @stop {{-- local styles --}} @section('header_styles')
                    ...
                    <div class="col-md-12">
						<form id="form" enctype="multipart/form-data" class="form-horizontal form-bordered">
							...
							<div class="form-group striped-col">
				                <label class="col-sm-3 control-label">KMP Slum Program</label>
				                <div class="col-sm-6">
									<select id="kode_kmp_slum_prog-input" name="kode_kmp_slum_prog-input" class="form-control" size="1" required>
										<option value>Please Select</option>
										@foreach ($kmp_slum_list as $kpl)
				                            <option value="{{$kpl->kode}}" {!! $kode_kmp_slum_prog==$kpl->kode ? 'selected':'' !!}>{{$kpl->nama}}</option>
				                        @endforeach
									</select>
				                </div>-
				            </div>
                            ...
                            <div class="form-group form-actions">
                                <div class="col-sm-9 col-sm-offset-3">
                                    <a href="/hrm/management/kuota/kmp" type="button" class="btn btn-effect-ripple btn-danger">
                                        Cancel
                                    </a>
...
<script>
  $(document).ready(function () {
	...
	$('#form').on('submit', function (e) {
		...
		$.ajax({
			type: 'post',
			"url": "/hrm/management/kuota/kmp/create",
			data: $('form').serialize(),
			beforeSend: function (){
			...
			window.location.href = "/hrm/management/kuota/kmp";
			},
	...
	$("#kode_kmp_slum_prog-input").select2({
		theme: "bootstrap"
	});
