...
class bk020320Controller extends Controller
{
    /**
    ...
    public function index()
    {
		...
		if(count($akses) > 0){
			foreach ($akses as $item) {
				...
				if($item->kode_menu==219)
					$data['detil'][$item->kode_menu_detil]='a';
			}
			if(!empty($data['detil'])){
			    $data['username'] = $user->name;
				$this->log_aktivitas('View', 685);
				return view('HRM/bk020320/index',$data);
			}
			else {
				return Redirect::to('/');
			}
	...
	public function Post(Request $request)
	{
		...
		if(!empty($posts))
		{
			foreach ($posts as $post)
			{
				...
				$edit =  $post->kode;
				$url_edit="/hrm/management_personil/persetujuan/evaluasi/create?kode=".$edit;
	...
	public function create(Request $request)
	{
		...
		if(count($akses) > 0){
			...
			if($data['kode']!=null && !empty($data['detil']['687'])){
				...
				return view('HRM/bk020320/create',$data);
			}else {
				return Redirect::to('/');
			}
		}else{
