...
class bk020321Controller extends Controller
{
    /**
    ...
    public function index()
    {
		...
		if(count($akses) > 0){
			foreach ($akses as $item) {
				...
				if($item->kode_menu==221)
					$data['detil'][$item->kode_menu_detil]='a';
			}
			if(!empty($data['detil'])){
			    $data['username'] = $user->name;
				$this->log_aktivitas('View', 693);
				return view('HRM/bk020321/index',$data);
			}
			else {
				return Redirect::to('/');
			}
	...
	public function Post(Request $request)
	{
		...
		if(!empty($posts))
		{
			...
			{
				$edit =  $post->kode;
				$url_edit="/hrm/management_personil/mutasi/create?kode=".$edit;
				$url_delete="/hrm/management_personil/mutasi/delete?kode=".$edit;
				$url_approve="/hrm/management_personil/mutasi/approve?kode=".$edit;
				...
				if(!empty($detil['696']) && empty($nestedData['divalidasi_oleh']) && $user->id==$post->kode_user){
	...
	public function create(Request $request)
	{
		...
		if(count($akses) > 0){
			...
			$data['kota_baru_list']=[];
			$data['kel_baru_list']=[];
			if($data['kode']!=null && !empty($data['detil']['695'])){
				...
				return view('HRM/bk020321/create',$data);
			}else if($data['kode']==null && !empty($data['detil']['694'])){
