...
class bk020317Controller extends Controller
{
    /**
    ...
    public function index()
    {
		...
		if(count($akses) > 0){
			foreach ($akses as $item) {
				...
				if($item->kode_menu==215)
					$data['detil'][$item->kode_menu_detil]='a';
			}
			if(!empty($data['detil'])){
			    $data['username'] = $user->name;
				$this->log_aktivitas('View', 669);
				return view('HRM/bk020317/index',$data);
			}
			else {
				return Redirect::to('/');
			}
	...
	public function Post(Request $request)
	{
		...
		if(!empty($posts))
		{
			...
			{
				$edit =  $post->kode;
				$url_edit="/hrm/management_personil/persetujuan/mutasi/edit?kode=".$edit;
	...
	public function create(Request $request)
	{
		...
		if(count($akses) > 0){
			...
			if($data['kode']!=null && !empty($data['detil']['671'])){
				...
				return view('HRM/bk020317/create',$data);
			}else {
				return Redirect::to('/');
			}
		}else{
