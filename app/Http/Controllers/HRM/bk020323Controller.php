...
class bk020322Controller extends Controller
{
    ...
    public function index()
    {
		...
		if(count($akses) > 0){
			foreach ($akses as $item) {
				...
				if($item->kode_menu==223)
					$data['detil'][$item->kode_menu_detil]='a';
			}
			if(!empty($data['detil'])){
			    $data['username'] = $user->name;
				$this->log_aktivitas('View', 701);
	...
	public function Post(Request $request)
	{
		...
		if(!empty($posts))
		{
			...
			{
				$edit =  $post->kode;
				$url_edit="/hrm/management_personil/promosi/create?kode=".$edit;
				$url_delete="/hrm/management_personil/promosi/delete?kode=".$edit;
				$url_approve="/hrm/management_personil/promosi/approve?kode=".$edit;
				...
				if(!empty($detil['704']) && empty($nestedData['divalidasi_oleh']) && $user->id==$post->kode_user){
	...
	public function create(Request $request)
	{
		...
		if(count($akses) > 0){
			...
			$data['kota_baru_list']=[];
			$data['kel_baru_list']=[];
			if($data['kode']!=null && !empty($data['detil']['703'])){
			...
			}else if($data['kode']==null && !empty($data['detil']['702'])){
