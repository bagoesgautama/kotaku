...
class bk020202Controller extends Controller
{
    /**
    ...
    public function index()
    {
		...
		if(count($akses) > 0){
			foreach ($akses as $item) {
				...
				if($item->kode_menu==209)
					$data['detil'][$item->kode_menu_detil]='a';
			}
			if(!empty($data['detil'])){
			    $data['username'] = $user->name;
				$this->log_aktivitas('View', 653);
				return view('HRM/bk020202/index',$data);
			}
			else {
				return Redirect::to('/');
			}
	...
	public function Post(Request $request)
	{
		...
		if(!empty($posts))
		{
			...
			{
				$edit =  $post->kode;
				$url_edit="/hrm/management_diri/mutasi/create?kode=".$edit;
				$url_delete="/hrm/management_diri/mutasi/delete?kode=".$edit;
				$url_approve="/hrm/management_diri/mutasi/approve?kode=".$edit;
				...
				if(!empty($detil['656']) && empty($nestedData['divalidasi_oleh']) && $user->id==$post->kode_user){
	...
	public function create(Request $request)
	{
		...
		if(count($akses) > 0){
			...
			$data['kota_baru_list']=[];
			$data['kel_baru_list']=[];
			if($data['kode']!=null && !empty($data['detil']['655'])){
				$rowData = DB::select('select * from bkt_02030204_perubahan_status where kode_jns_perubahan=3 and kode='.$data['kode']);
				...
				return view('HRM/bk020202/create',$data);
			}else if($data['kode']==null && !empty($data['detil']['654'])){
	...
	public function post_create(Request $request)
	{
		...
		if ($request->input('kode')!=null){
			...
			if($upload == true){
				$file->move(public_path('/uploads/mutasi'), $user->id."_".$file->getClientOriginalName());
			}
			if($upload2 == true){
				$file2->move(public_path('/uploads/mutasi'), $user->id."_".$file2->getClientOriginalName());
			}
			if($upload3 == true){
				$file3->move(public_path('/uploads/mutasi'), $user->id."_".$file3->getClientOriginalName());
			}
			$this->log_aktivitas('Update', 655);

		}else{
			DB::table('bkt_02030204_perubahan_status')->insert(
       			['kode_jns_perubahan' => 3,
				'kode_role_lama' => $request->input('kode_role_lama'),
				'kode_role_baru' => $request->input('kode_role_baru-input'),
				'tgl_efektif_role_baru' => $request->input('tgl_efektif_role_baru-input'),
				'catatan' => $request->input('catatan-input'),
	...
	public function delete(Request $request)
	{
        ...
        $this->log_aktivitas('Delete', 656);
        return Redirect::to('/hrm/management_diri/mutasi');
    }
