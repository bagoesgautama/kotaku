...
class bk010408Controller extends Controller
{
	...
	public function select(Request $request)
	{
		...
		if(!empty($request->input('kode_parent_kawasan'))){
			$kawasan = DB::select('select a.kode_kawasan, b.id, b.kode_kawasan, b.nama from bkt_01040201_real_keg a, bkt_01010123_kawasan b where a.kode_kawasan=b.id and a.kode_parent='.$request->input('kode_parent_kawasan'));
			echo json_encode($kawasan);
		}
		if(!empty($request->input('kode_kegiatan_parent'))){
			$kegiatan = DB::select('select * from bkt_01030208_usulan_keg_kt where kode='.$request->input('kode_kegiatan_parent'));
	...
	public function create(Request $request)
	{
		...
		if(count($akses) > 0){
			...
			$data['kode_kawasan']=$request->input('kode_kawasan');
			if($data['kode']!=null && $data['kode_kawasan']!=null && !empty($data['detil']['376'])){
				$rowData = DB::select('select * from bkt_01040201_real_keg where kode='.$data['kode']. 'and kode_kawasan='.$data['kode_kawasan']);
				...
				if(!empty($rowData[0]->kode_parent))
					$data['kode_kmw_list']=DB::select('select b.kode, b.nama from bkt_01040201_real_keg a, bkt_01010110_kmw b where a.kode_kmw=b.kode and a.kode='.$rowData[0]->kode_parent);
				if(!empty($rowData[0]->kode_parent))
				...
				if(!empty($rowData[0]->kode_parent))
					$data['kode_kawasan_list']=DB::select('select a.kode_kawasan, b.id, b.kode_kawasan, b.nama from bkt_01040201_real_keg a, bkt_01010123_kawasan b where a.kode_kawasan=b.id and a.kode_parent='.$rowData[0]->kode_parent);
			...
			}else if ($data['kode']==null && $data['kode_kawasan']==null && !empty($data['detil']['375'])){
