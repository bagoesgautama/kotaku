...
class bk010414Controller extends Controller
{
    /**
    ...
    public function index()
    {
		...
		if(count($akses) > 0){
			...
			if(!empty($data['detil'])){
				...
				return view('MAIN/bk010414/index',$data);
			}
			else {
				return Redirect::to('/');
			}
    ...
    public function select(Request $request)
	{
		if(!empty($request->input('kode_parent'))){
			...
			from bkt_01040202_real_ktrk_krj a
				...
				left join bkt_01030213_ktrk_pkt_krj_kontraktor h on h.kode=a.kode_parent 
	...
	public function Post(Request $request)
	{
		...
		if(!empty($posts))
		{
			foreach ($posts as $post)
			{
				...
				$url_edit=url('/')."/main/pelaksanaan/kota_bdi/realisasi_kontrak/sertias/create?kode=".$edit;
				$url_delete=url('/')."/main/pelaksanaan/kota_bdi/realisasi_kontrak/sertias/delete?kode=".$delete;
	...
	public function create(Request $request)
	{
		...
		if(count($akses) > 0){
			...
			if($data['kode']!=null  && !empty($data['detil']['356'])){
				...
				// 	$data['kode_kmw_list']=DB::select('select b.kode, b.nama from bkt_01040202_real_ktrk_krj a, bkt_01010110_kmw b where a.kode_kmw=b.kode and a.kode='.$request->input('kode'));
				// if(!empty($request->input('kode')))
				// 	$data['kode_kota_list']=DB::select('select b.kode, b.nama from bkt_01040202_real_ktrk_krj a, bkt_01010102_kota b where a.kode_kota=b.kode and a.kode='.$data['kode']);
				// if(!empty($request->input('kode')))
				// 	$data['kode_korkot_list']=DB::select('select b.kode, b.nama from bkt_01040202_real_ktrk_krj a, bkt_01010111_korkot b where a.kode_korkot=b.kode and a.kode='.$request->input('kode'));
				// if(!empty($request->input('kode')))
				// 	$data['kode_kawasan_list']=DB::select('select b.id, b.kode_kawasan, b.nama from bkt_01040202_real_ktrk_krj a, bkt_01010123_kawasan b where a.kode_kota=b.kode_kota and a.kode='.$request->input('kode'));
				...
				return view('MAIN/bk010414/create',$data);
			}else if ($data['kode']==null  && !empty($data['detil']['356'])){
	...
	public function post_create(Request $request)
	{
		...
		DB::table('bkt_01040202_real_ktrk_krj')->where('kode', $request->input('kode-parent-input'))
		->update([
	...
	public function delete(Request $request)
	{
		...
		$this->log_aktivitas('Delete', 356);
        return Redirect::to('/main/pelaksanaan/kota_bdi/realisasi_kontrak/sertias');
    }
