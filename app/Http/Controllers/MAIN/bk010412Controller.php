...
class bk010412Controller extends Controller
{
    /**
    ...
    public function index()
    {
		...
		if(count($akses) > 0){
			...
			if(!empty($data['detil'])){
				...
				$this->log_aktivitas('View', 390);
				return view('MAIN/bk010412/index',$data);
			}
			else {
				return Redirect::to('/');
			}
	...
	public function Post(Request $request)
	{
		...
		if(!empty($posts))
		{
			foreach ($posts as $post)
			{
				...
				$url_edit=url('/')."/main/pelaksanaan/kelurahan_non/sertifikasi_infra/create?kode=".$edit;
				$url_delete=url('/')."/main/pelaksanaan/kelurahan_non/sertifikasi_infra/delete?kode=".$delete;
	...
	public function create(Request $request)
	{
		...
		if(count($akses) > 0){
			...
			return view('MAIN/bk010412/create',$data);
		}
	}
