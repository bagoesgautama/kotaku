...
class bk010302Controller extends Controller
{
    /**
    ...
    public function index()
    {
		...
		if(count($akses) > 0){
			foreach ($akses as $item) {
				...
				if($item->kode_menu==90)
					$data['detil'][$item->kode_menu_detil]='a';
			}
			if(!empty($data['detil'])){
			    $data['username'] = $user->name;

				$this->log_aktivitas('View', 257);
				return view('MAIN/bk010302/index',$data);
			}
			else {
				return Redirect::to('/');
			}
	...
	public function Post(Request $request)
	{
		...
		$query='select a.kode, a.tahun, b.nama as kode_kota, d.nama as kode_kmw, e.nama as kode_korkot, a.jenis_kegiatan, a.tgl_kegiatan, a.lok_kegiatan, a.created_time from bkt_01030201_plan_pmkm_kt a, bkt_01010102_kota b, bkt_01010110_kmw d, bkt_01010111_korkot e where a.kode_kota = b.kode and a.kode_kmw = d.kode and a.kode_korkot = e.kode and a.jenis_kegiatan = "3.1.2"';
		$totalData = DB::select('select count(1) cnt from bkt_01030201_plan_pmkm_kt a, bkt_01010102_kota b, bkt_01010110_kmw d, bkt_01010111_korkot e where a.kode_kota = b.kode and a.kode_kmw = d.kode and a.kode_korkot = e.kode and a.jenis_kegiatan = "3.1.2"');
		...
		if(!empty($posts))
		{
			foreach ($posts as $post)
			{
				...
				$url_edit=url('/')."/main/perencanaan/penanganan/pelaksanaan_rpk/create?kode=".$edit;
				$url_delete=url('/')."/main/perencanaan/penanganan/pelaksanaan_rpk/delete?kode=".$delete;
	...
	public function create(Request $request)
	{
		...
		if(count($akses) > 0){
			...
			if($data['kode']!=null  && !empty($data['detil']['259'])){
	...
	public function post_create(Request $request)
	{
		...
		if ($request->input('kode')!=null){
			...
			if($upload_dokumen == true){
				$file_dokumen->move(public_path('/uploads/perencanaan/penanganan/pelaksanaan_rpk'), $file_dokumen->getClientOriginalName());
			}

			if($upload_absensi == true){
				$file_absensi->move(public_path('/uploads/perencanaan/penanganan/pelaksanaan_rpk'), $file_absensi->getClientOriginalName());
			}

			$this->log_aktivitas('Update', 259);
	...
	public function delete(Request $request)
	{
		...
		$this->log_aktivitas('Delete', 260);
        return Redirect::to('/main/perencanaan/penanganan/pelaksanaan_rpk');
    }
