...
class bk010121Controller extends Controller
{
	...
	public function Post(Request $request)
	{
		...
		if(empty($request->input('search.value')))
		{
			...
			$totalFiltered=DB::select('select count(1) from ('.$query. ' and aspek like "%'.$search.'%" ) a');
		}
		...
		if(!empty($posts))
		{
			...
			{
				$edit =  $post->id;
				$url_edit=url('/')."/main/data_master/aspek_kumuh/create?id=".$edit;
				$url_delete=url('/')."/main/data_master/aspek_kumuh/delete?id=".$edit;
