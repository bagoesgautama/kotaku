...
class bk010210Controller extends Controller
{
	...
	public function Post(Request $request)
	{
		...
		if(empty($request->input('search.value')))
		{
			...
			$posts=DB::select($query. ' where (
				...
				nama_kota like "%'.$search.'%" or
				a.tgl_kegiatan like "%'.$search.'%") order by '.$order.' '.$dir.' limit '.$start.','.$limit);
			$totalFiltered=DB::select('select count(1) cnt from ('.$query. ' where (
				a.tahun like "%'.$search.'%" or
				nama_kota like "%'.$search.'%" or
				a.tgl_kegiatan like "%'.$search.'%")) a');
		...
		if(!empty($posts))
		{
			foreach ($posts as $post)
			{
				$edit =  $post->kode;
				$delete = $post->kode;
				$url_show="/main/persiapan/kota/forum/kolaborasi/show?kode=".$edit;
				$url_edit="/main/persiapan/kota/forum/kolaborasi/create?kode=".$edit;
				$url_delete="/main/persiapan/kota/forum/kolaborasi/delete?kode=".$delete;
	...
	public function show(Request $request)
	{
		...
		if(count($akses) > 0){
			...
			if($data['kode']!=null && !empty($data['detil']['115'])){
				$data['detil_menu']='115';
