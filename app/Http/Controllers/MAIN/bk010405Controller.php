...
class bk010405Controller extends Controller
{
    /**
    ...
    public function index()
    {
		...
		if(count($akses) > 0){
			...
			if(!empty($data['detil'])){
				...
				$this->log_aktivitas('View', 343);
				return view('MAIN/bk010405/index',$data);
			}
			else {
				return Redirect::to('/');
			}
	...
	public function Post(Request $request)
	{
		...
		if(!empty($posts))
		{
			foreach ($posts as $post)
			{
				...
				$url_edit=url('/')."/main/pelaksanaan/kota_non/realisasi_kegiatan/create?kode=".$edit;
				$url_delete=url('/')."/main/pelaksanaan/kota_non/realisasi_kegiatan/delete?kode=".$delete;
	...
	public function Post_pemanfaat(Request $request)
	{
		if($request->input('kode')!=null)
		{
			...
			if(empty($request->input('search.value')))
			{
				...
				$totalFiltered=DB::select('select count(1) cnt from ('.$query. ' and (a.nik like "%'.$search.'%" or a.nama like "%'.$search.'%" or a.alamat like "%'.$search.'%" or a.kode_jenis_kelamin like "%'.$search.'%")) a');
				$totalFiltered = $totalFiltered[0]->cnt;
			}
			...
			if(!empty($posts))
			{
				foreach ($posts as $post)
				{
					...
					$url_delete=url('/')."/main/pelaksanaan/kota_non/realisasi_kegiatan/pemanfaat/delete?kode=".$delete."&kode_real_keg=".$request->input('kode');
		...
		elseif($request->input('kode_real_keg')!=null)
		{
			...
			if(empty($request->input('search.value')))
			{
				...
				if($request->input('where')!=null){
					...
					$totalFiltered=DB::select('select count(1) cnt from ('.$query. ' and (nik like "%'.$search.'%" or nama like "%'.$search.'%" or alamat like "%'.$search.'%" or kode_jenis_kelamin like "%'.$search.'%")) a');
				}else{
					$posts=DB::select($query. ' where (nik like "%'.$search.'%" or nama like "%'.$search.'%" or alamat like "%'.$search.'%" or kode_jenis_kelamin like "%'.$search.'%") order by '.$order.' '.$dir.' limit '.$start.','.$limit);
					$totalFiltered=DB::select('select count(1) cnt from ('.$query. ' where (nik like "%'.$search.'%" or nama like "%'.$search.'%" or alamat like "%'.$search.'%" or kode_jenis_kelamin like "%'.$search.'%")) a');
				}
				$totalFiltered = $totalFiltered[0]->cnt;
			}
	...
	public function create(Request $request)
	{
		...
		if(count($akses) > 0){
			...
			if($data['kode']!=null  && !empty($data['detil']['531'])){
				...
				return view('MAIN/bk010405/create',$data);
			}else if ($data['kode']==null  && !empty($data['detil']['530'])){
	...
	public function pemanfaat_create(Request $request)
	{
		...
		if(count($akses) > 0){
			...
			if($data['kode_real_keg']!=null  && !empty($data['detil']['531'])){
				...
				$data['where'] = $where; 
				return view('MAIN/bk010405/pemanfaat',$data);
			}
		}
	}
	...
	public function post_pemanfaat_create(Request $request)
	{
		if($request->input('kode_real_keg')!=null){
			...
			$this->log_aktivitas('Update Penerima Manfaat', 531);
		}
	}
	...
	public function post_create(Request $request)
	{
		...
		if ($request->input('kode')!=null){
			...
			if($upload_prcn0 == true){
				$file_prcn0->move(public_path('/uploads/pelaksanaan/Realisasi_kegiatan_skala_kota_non_BDI_kolaborasi/realisasi_kegiatan_skala_kota'), $file_prcn0->getClientOriginalName());
			}

			if($upload_prcn50 == true){
				$file_prcn50->move(public_path('/uploads/pelaksanaan/Realisasi_kegiatan_skala_kota_non_BDI_kolaborasi/realisasi_kegiatan_skala_kota'), $file_prcn50->getClientOriginalName());
			}

			if($upload_prcn100 == true){
				$file_prcn100->move(public_path('/uploads/pelaksanaan/Realisasi_kegiatan_skala_kota_non_BDI_kolaborasi/realisasi_kegiatan_skala_kota'), $file_prcn100->getClientOriginalName());
			}

			$this->log_aktivitas('Update', 531);
	...
	public function delete(Request $request)
	{
		...
		$this->log_aktivitas('Delete', 532);
        return Redirect::to('/main/pelaksanaan/kota_non/realisasi_kegiatan');
    }
    ...
    public function delete_pemanfaat(Request $request)
	{
        ...
        return Redirect::to('/main/pelaksanaan/kota_non/realisasi_kegiatan/create?kode='.$request->input('kode_real_keg'));
    }
