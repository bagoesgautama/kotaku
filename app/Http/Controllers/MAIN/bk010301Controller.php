...
class bk010301Controller extends Controller
{
	...
	public function Post(Request $request)
	{
		...
		if(!empty($posts))
		{
			foreach ($posts as $post)
			{
				...
				$delete = $post->kode;
				//show
				$url_show=url('/')."/main/perencanaan/penanganan/pembangunan_visi/show?kode=".$edit;
	...
	public function show(Request $request)
	{
		...
		if(count($akses) > 0){
			...
			if($data['kode']!=null  && !empty($data['detil']['253'])){
