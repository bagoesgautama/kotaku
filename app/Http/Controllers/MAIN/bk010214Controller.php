...
class bk010214Controller extends Controller
{
			...
			if(!empty($data2['detil'])){
				...
				if(!empty($posts))
				{
					foreach ($posts as $post)
					{
						$edit =  $post->kode;
						$delete = $post->kode;
						$url_show="/main/persiapan/kecamatan/keberfungsian/show?kode=".$edit;
						$url_edit="/main/persiapan/kecamatan/keberfungsian/create?kode=".$edit;
						$url_delete="/main/persiapan/kecamatan/keberfungsian/delete?kode=".$delete;
	...
	public function show(Request $request)
	{
		...
		if(count($akses) > 0){
			...
			if($data['kode']!=null && !empty($data['detil']['119'])){
				$data['detil_menu']='119';
	...
	public function post_create(Request $request)
	{
		...
		if ($request->input('kode')!=null){
			...
			if($upload_document == true){
				$file_document->move(public_path('/uploads/persiapan/kecamatan/forum/keberfungsian'), $file_document->getClientOriginalName());
			}
