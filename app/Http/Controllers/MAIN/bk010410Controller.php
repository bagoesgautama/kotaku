...
class bk010410Controller extends Controller
{
    ...
    public function post_peserta(Request $request)
	{
		...
		$query='select * from bkt_01040206_real_keg_pmft ';
			
		$totalData = DB::select('select count(1) cnt from bkt_01040206_real_keg_pmft ');
		$totalFiltered = $totalData[0]->cnt;
		$limit = $request->input('length');
		$start = $request->input('start');
		$order = $columns[$request->input('order.0.column')];
		...
		if(!empty($posts))
		{
			foreach ($posts as $post)
			{
				...
				$nestedData['kode_real_keg'] = $post->kode_real_keg;
				$nestedData['kode_pemanfaat'] = $post->kode_pemanfaat;
				$nestedData['created_time'] = $post->created_time;
	...
	public function create_peserta(Request $request)
	{
		...
		if(count($akses) > 0){
			...
			if($data['kode']!=null  && !empty($data['detil']['303'])){
				$rowData = DB::select('select * from bkt_01040206_real_keg_pmft where kode='.$data['kode']);
				$data['kode_real_keg'] = $rowData[0]->kode_real_keg;
				$data['kode_pemanfaat'] = $rowData[0]->kode_pemanfaat;
				$data['created_time'] = $rowData[0]->created_time;
				$data['kode_user_list'] = DB::select('select * from bkt_02010111_user');
				return view('MAIN/bk010410/create_peserta',$data);
			}else if ($data['kode']==null  && !empty($data['detil']['302'])){
				$data['kode_real_keg'] = null;
				$data['kode_pemanfaat'] = null;
				$data['created_time'] = null;
				$data['kode_user_list'] = DB::select('select * from bkt_02010111_user');
				return view('MAIN/bk010410/create_peserta',$data);
			}else{
	...
	public function post_create_peserta(Request $request)
	{
		if ($request->input('kode')!=null){
			date_default_timezone_set('Asia/Jakarta');
			DB::table('bkt_01040206_real_keg_pmft')->where('kode', $request->input('kode'))
			->update(['kode_real_keg' => $request->input('kode_real_keg-input'),
				'kode_pemanfaat' => $request->input('kode_pemanfaat-input'),
				]);
			...
			DB::table('bkt_01040206_real_keg_pmft')->insert([
				'kode_real_keg' => $request->input('kode_real_keg-input'),
				'kode_pemanfaat' => $request->input('kode_pemanfaat-input'),
       			]);
