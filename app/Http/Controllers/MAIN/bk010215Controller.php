...
class bk010215Controller extends Controller
{
	...
	public function select(Request $request)
	{
		...
		elseif(!empty($request->input('data_kel'))){
			$data_kel = DB::select('select count(a.id) sum_rt, sum(b.D16) sum_penduduk, sum(b.D15) sum_penduduk_wanita, sum(b.D11) sum_kk, sum(b.D12) sum_kk_mbr, sum(b.D13) sum_kk_miskin, count(b.D9) sum_rt_kmh, count(c.id) sum_rw from bkt_01010116_rt a, bkt_00010101_baseline b, bkt_01010132_rw c where a.kode_kel=b.kode_kel and a.status ="1" and b.kode_kel=c.kode_kel and c.status ="1" and a.kode_kel='.$request->input('data_kel'));
